...
def run_simulation(
    ...
    """
    Run one simulation scenario and return a list of metric snapshots
    logged every 60 seconds.

    Args:
        use_optimization: True → ML-adaptive timings; False → fixed 30-30
    ...
    if use_optimization:
        ...
        timings = optimize_network(rates)
    else:
    ...
    for step in range(total_steps):
        ...
        # --- Actual arrival rates from ground-truth pattern ---
...
def main():
    ...
    print('  ML (GradientBoosting) + Math Opt (Webster\'s formula)')
    print(banner)
    ...
    print(f'  CV RMSE: {metrics_ml["cv_rmse_mean"]:.1f} ± {metrics_ml["cv_rmse_std"]:.1f} veh/hr'
    ...
    print('\n  --- OPTIMIZED (ML-adaptive Webster timing) ---')
    ...
    if not args.no_plots:
        ...
        # Signal timings across 24 h
        timings_history = []
        for h in range(24):
            rates = predictor.predict_rates(h, args.day_of_week, args.weather)
            timings_history.append({'hour': h, 'timings': optimize_network(rates)})
